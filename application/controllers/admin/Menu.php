...
class Menu extends CI_Controller {
	...
	public function update($id_menu)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			}elseif ($jenis_link=="agenda") {
				$data_link = base_url('agenda/').$link_agenda;
			}else{
				$data_link = $link_url.$link_url2;
			}
			...
			$this->menu_model->update($id_menu, $judul, $induk, $kategori_menu, $jenis_link, $urut, $data_link);
	...
	{		
		...
		$data['pil_kategori'] 		= $this->artikel_model->get_kategori_artikel_by_user_id($user_id);
		$data['pil_agenda']			= $this->agenda_model->get_all_agenda();
	...
	public function simpan()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->menu_model->simpan($judul, $induk, $kategori_menu, $jenis_link, $urut, $data_link);
